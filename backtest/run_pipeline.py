...
promoting Stage 3's certified configurations with `--auto-promote`.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.auto_promote:
        rc = auto_promote(strat, out_dir=out_dir, dry_run=dry)
...
def auto_promote(strat: str, *, out_dir: str | None = None,
    ...
    fails is reported and the rest continue: the certifications are already on
    ...
    for r in refused:
        print(f"  NOT PROMOTED  {r.get('symbol')} {r.get('timeframe')} "
              f"version {r.get('version')} · status "
              f"{r.get('status', 'UNKNOWN')} · Gate R "
              f"{r.get('gate_regime', 'NOT EVALUATED')}")
    if not winners:
    ...
    for r in winners:
        ...
        if not audit:
            print(f"  SKIPPED  {r.get('symbol')} {r.get('timeframe')}: the row "
                  f"records no audit file, so there is no certification to "
                  f"cite.", file=sys.stderr)
            failures += 1
            continue
        cmd = promote_cmd(strat, str(r.get("version") or "A"), source, audit,
                          str(r.get("symbol")), str(r.get("timeframe")))
        rc = run_step(cmd, f"PROMOTE {r.get('symbol')} {r.get('timeframe')} "
                           f"version {r.get('version')}", check=False)
        if rc:
