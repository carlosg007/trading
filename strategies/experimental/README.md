...
`bars` is ONE instrument's OHLCV frame, oldest to newest — the engine calls it
once per symbol. `write_and_validate_strategy` still appends an adapter, but
since 2026-08-15 it no longer reshapes arguments. It exists for the two things
the model cannot be trusted with:

1. **Parameter binding.** The engine calls `signal_fn(bars)` with no params, so
   a parameterised module needs a `make_signal_fn(**params)` factory for
   `load_strategy` to bind against.
2. **Boolean coercion.** A model that returns a price series instead of a
   comparison must fail loudly rather than open a position on every bar.
